...
const Header = ({getSearchNews}:Props) => {
    ...
    const subscribeTopic=async(topic:string,item:string)=>{
        ...
        try{
            setloading(item)
            const data=await axios.put('https://6408b80201472000081e9bcc--beautiful-rolypoly-1da010.netlify.app/.netlify/functions/user/subscribetopic',{topic},{headers:{authorisation:`Bearer ${token}`}})
    ...
    const unsubscribeTopic=async(topic:string,item:string)=>{
        ...
        try{
            setloading(item)
            const data=await axios.put('https://6408b80201472000081e9bcc--beautiful-rolypoly-1da010.netlify.app/.netlify/functions/user/unsubscribetopic',{topic},{headers:{authorisation:`Bearer ${token}`}})
            setUser(data.data.user)
            setloading('')
        }
        catch(err:any){
    ...
    const unsubscribeAuthor=async(author:string,item:string)=>{
        ...
        try{
            setloading(item);
            const data=await axios.put('https://6408b80201472000081e9bcc--beautiful-rolypoly-1da010.netlify.app/.netlify/functions/user/unsubscribeauthor',{author},{headers:{authorisation:`Bearer ${token}`}})
            setUser(data.data.user)
            setloading('')
        }
        catch(err:any){
